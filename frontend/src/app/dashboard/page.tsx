...
import heroImage from "../../pictures/header-woman.png"
...
const GeneSurvivePage: React.FC = () => {
  const teamMembers = [
  ...
  return (
    <div className="font-sans">
      {/* Navbar */}
      <nav className="bg-pink-700 text-white p-4 flex justify-between">
        <h1 className="text-xl font-bold">GeneSurvive</h1>
      ...
      <section className="py-12 flex flex-col items-start justify-center text-left px-8 md:px-16 min-h-[500px] relative overflow-hidden">
        {/* Gradient Background - same as team section */}
        ...
        {/* Soft overlay for additional transparency */}
        <div className="absolute inset-0 bg-white/20"></div>

        <div className="max-w-lg z-10 relative">
          <h2 className="text-5xl md:text-6xl font-bold text-pink-600 mb-4 leading-tight">GeneSurvive</h2>
          <p className="text-xl md:text-2xl text-gray-800 font-semibold mb-2">Predict Breast Cancer Survivability</p>
          <p className="text-lg text-pink-500 mb-6">Based on Gene Expression</p>
          <button className="bg-green-400 text-black px-8 py-3 rounded-full hover:bg-green-500 font-semibold">
            Predict Now!
          </button>
        </div>
        {/* Pink ribbon decoration */}
        <div className="absolute bottom-8 left-8 z-10">
          <div className="w-16 h-20 bg-pink-500 rounded-t-full transform rotate-12 opacity-80"></div>
        </div>
      </section>

      {/* About Section */}
      <section id="about" className="py-12 bg-pink-50 text-center">
        <h3 className="text-3xl font-bold text-gray-800 mb-8 uppercase tracking-wide">About Project</h3>
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center">
          <img src={illustration.src || "/placeholder.svg"} alt="Illustration" className="w-64 mb-8 md:mb-0 md:mr-12" />
          <div className="text-left">
            <p className="text-gray-700 text-base leading-relaxed">
              GeneSurvive is a machine learning-based web application developed to predict the survivability of breast
              cancer patients using clinical data and gene expression profiles. Built on the METABRIC dataset, which
              contains 31 clinical features, 331 mRNA expression levels, and 175 gene mutation records, GeneSurvive
              helps estimate whether a patient is likely to survive, along with a confidence score. This tool aims to
              support research in personalized cancer prognosis and treatment strategies. While GeneSurvive provides
              valuable insights from data, it is intended for educational and research purposes only—not for clinical
              diagnosis.
            </p>
          </div>
        </div>
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute bottom-16 left-20 w-16 h-16 opacity-10">
            <svg viewBox="0 0 100 100" className="w-full h-full text-pink-600">
              <rect x="40" y="10" width="20" height="80" fill="currentColor" rx="4" />
              <rect x="10" y="40" width="80" height="20" fill="currentColor" rx="4" />
            </svg>
          </div>
      ...
      {/* Footer */}
      <footer className="bg-pink-700 text-white text-center py-4">@2025 GeneSurvive</footer>
    </div>
  )
}

export default GeneSurvivePage
